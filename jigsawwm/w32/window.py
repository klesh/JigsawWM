...
#   return args
# user32.GetWindowTextA.errcheck = errcheck_bool
...
class Window:
  ...
  def is_cloaked(self) -> any:
    val = INT(0)
    windll.dwmapi.DwmGetWindowAttribute(self._hwnd, DwmWindowAttribute.DWMWA_CLOAKED, pointer(val), sizeof(val))
    return val.value
...
def get_normal_windows(hdst: Optional[HDESK]=None) -> Iterator[Window]:
  ...
  for window in get_windows(hdst):
    ...
    if (
      window.title and
      not window.is_cloaked and
      WindowStyle.MAXIMIZEBOX & style and
      WindowStyle.MINIMIZEBOX & style and
      WindowStyle.VISIBLE & style and
      not WindowStyle.MINIMIZE & style
    ):
      yield window
